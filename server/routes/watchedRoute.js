...
router.post("/watchBulk", async (req, res) => {
  try {
    const { userId, userName, titles } = req.body;
        ...
        await db.collection("users").updateOne({ uid: userId }, [
          {
            $set: {
              totalWatched: {
                $max: [
                  { $subtract: [{ $ifNull: ["$totalWatched", 0] }, 1] },
                  0,
                ],
              },
            },
