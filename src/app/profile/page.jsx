...
export default function Profile() {
  ...
  const userRankedOne = user.rankings?.find((r) => r.rank === 1)?.title;
  const userRankedTwo = user.rankings?.find((r) => r.rank === 2)?.title;
  const userRankedThree = user.rankings?.find((r) => r.rank === 3)?.title;
          ...
          <div>
            <div className="flex gap-1 items-center">
              ...
              <p className="text-xs text-vibe">
                12/<span>{showNum}</span> watched
              </p>
            </div>
            {user.rankings ? (
              <div className="flex flex-col gap-1">
                <h4 className="bg-yellow-100 text-yellow-800 px-2 font-bold rounded-full">
                  <span className="text-2xl">1</span>
                  {userRankedOne}
                </h4>
                <h4 className="bg-gray-100 text-gray-800 px-2 font-bold rounded-full">
                  <span className="text-2xl">2</span>
                  {userRankedTwo}
                </h4>
                <h4 className="bg-orange-100 text-orange-800 px-2 font-bold rounded-full">
              ...
              <div>
                <p className="text-xs text-vibe">You have not ranked yet</p>
                <button
                  onClick={() => router.push("/rank")}
                  className="px-4 py-2 bg-accent cursor-pointer rounded-full w-full"
                >
                  <p className="font-bold text-normal">Rank'em</p>
                </button>
              </div>
            )}
          </div>
